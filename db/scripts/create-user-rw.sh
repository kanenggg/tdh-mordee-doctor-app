...
# Create (or update) a read-write user and grant appropriate privileges
# on the target database.
#
# Usage: create-user-rw.sh {db-name} {admin-user} {admin-pass} {rw-user} {rw-password}
#
# Args:
...
#   $4  rw-user      - Role name to create
#   $5  rw-password  - Password for the role
#
# Required env vars:
...
CMD="create-user-rw.sh {db-name} {admin-user} {admin-pass} {rw-user} {rw-password}"
...
[ -z "${RW_USER}" ]            && _fail "A rw-user parameter is not set. $CMD"
[ -z "${RW_PASS}" ]            && _fail "A rw-password parameter is not set. $CMD"
...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '${RW_USER}') THEN
    CREATE ROLE \"${RW_USER}\" WITH LOGIN PASSWORD '${RW_PASS}';
  ELSE
    ALTER ROLE \"${RW_USER}\" WITH LOGIN PASSWORD '${RW_PASS}';
  END IF;
END
\$\$;"
ret_val=$?
if [ "$ret_val" -ne 0 ]; then
  _fail "Failed to create/update read-write role '${RW_USER}'"
fi

# ---------------------------------------------------------------------------
# Step 2: Grant CONNECT on the database  (as superuser)
# ---------------------------------------------------------------------------
run-pgsql-cmd-as-postgres.sh "GRANT CONNECT ON DATABASE \"${DB_NAME}\" TO \"${RW_USER}\";"
ret_val=$?
if [ "$ret_val" -ne 0 ]; then
  _fail "Failed to grant CONNECT on database '${DB_NAME}' to '${RW_USER}'"
fi
...
ret_val=$?
if [ "$ret_val" -ne 0 ]; then
  _fail "Failed to grant schema/table privileges to '${RW_USER}'"
fi
...
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT USAGE, SELECT ON SEQUENCES TO \"${RW_USER}\";"

ret_val=$?
if [ "$ret_val" -ne 0 ]; then
  _fail "Failed to set default privileges for '${RW_USER}'"
fi
