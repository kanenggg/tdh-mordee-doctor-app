...
use crate::biz_apm::common::{BookingType, ConsultationChannel, PatientIdentity};
...
pub struct Medicine {
    ...
    #[serde(rename = "medicineNameTH")]
    pub medicine_name_th: String,
    #[serde(rename = "medicineNameEN")]
    pub medicine_name_en: String,
    #[serde(rename = "medicineInstructionEN")]
    pub medicine_instruction_en: String,
    #[serde(rename = "medicineInstructionTH")]
...
mod tests {
    ...
    // Exact payload captured from runtime logs (message_id=19549521427542392).
    #[test]
    fn test_deserialize_session_terminated_from_runtime_payload() {
        let raw = r#"{"__type":"SessionTerminated","bookingId":"2605295Z2HYF","patientIdentity":{"accountId":2495,"userProfileId":2540,"tenantId":1,"oidcUserId":"U0756f38532dc421269dfd913e3bd9b24"},"doctorId":300662,"terminationCode":{"__type":"SuccessfulSession","patientJoinedAt":1780022204,"doctorJoinedAt":1780022235},"terminatedBy":{"__type":"Doctor"},"terminatedAt":1780022246}"#;

        let event: ConsultationEvent = serde_json::from_str(raw).expect("should deserialize");
        match event {
    ...
    #[test]
    fn test_deserialize_termination_codes() {
        let codes = [
            (
                r#"{"__type":"SuccessfulSession","patientJoinedAt":1780022204,"doctorJoinedAt":1780022235}"#,
                "SuccessfulSession",
            ),
            (
                r#"{"__type":"PatientAbsent","doctorJoinedAt":1780022235}"#,
                "PatientAbsent",
            ),
            (
                r#"{"__type":"DoctorAbsent","patientJoinedAt":1780022204}"#,
                "DoctorAbsent",
            ),
            (r#"{"__type":"BothPartiesAbsent"}"#, "BothPartiesAbsent"),
            (
                r#"{"__type":"TechnicalError","errorMessage":"network"}"#,
                "TechnicalError",
            ),
        ];

        for (raw, expected_type) in codes {
            let code: TerminationCode = serde_json::from_str(raw).unwrap();
            let value = serde_json::to_value(code).unwrap();
            assert_eq!(value["__type"], expected_type);
        }
    }

    // Exact payload captured from runtime logs (message_id=19545148375071494).
    #[test]
    fn test_deserialize_consultation_summarized_from_runtime_payload() {
        let raw = r#"{"__type":"ConsultationSummarized","bookingId":"2605295Z2HYF","patientIdentity":{"accountId":2495,"userProfileId":2540,"tenantId":1,"oidcUserId":"U0756f38532dc421269dfd913e3bd9b24"},"doctorId":300662,"doctorNote":"test test test\n","prescriptionInfo":{"prescriptionRefcode":"2605295Z2HYF","medicineItems":[{"pricePlanId":0,"medicineId":4,"medicineNameEN":"SARA Paracetamol 250 mg/5ml Orange SYRUP (60 ml/bottle)","medicineNameTH":"SARA Paracetamol 250 mg/5ml Orange SYRUP (60 ml/bottle)","medicineInstructionEN":"3 hours","medicineInstructionTH":"3 hours","medicineImageUrl":"https://example.com/26422.png","medicineAmount":5,"medicineUnitPrice":12.0}],"expireAt":1780108796},"summarizedAt":1780022418}"#;

        let event: ConsultationEvent = serde_json::from_str(raw).expect("should deserialize");
        match event {
            ConsultationEvent::ConsultationSummarized(e) => {
                let med = &e.prescription_info.medicine_items[0];
                assert_eq!(med.medicine_id, 4);
                assert!(med.medicine_name_en.starts_with("SARA"));
                assert!(med.medicine_name_th.starts_with("SARA"));
            }
            other => panic!("unexpected variant: {:?}", other),
        }
    }
